...
export type BrandAnalyticsStatus =
	| "PENDING"
	| "PROCESSING"
	| "COMPLETED"
	| "FAILED";
...
// BrandAnalyticsConfig is now imported from types/config.ts
...
/**
 * Market Basket Analysis (alias for compatibility)
 */
export type MarketBasketAnalysis = MarketBasketAnalysisResponse;

/**
 * Search Query Performance interface
 */
export interface SearchQueryPerformance {
	...
	competitiveMetrics: {
		marketShare: number;
		averageCompetitorPosition: number;
		gapToTopCompetitor: number;
	};
	trends: {
		weekOverWeek: number;
